...
import static com.chinalife.samy.ucr.Comparable.mathEx.BigDecimal.INFLATED;
...
class MutableBigInteger {
	 ...
	 * magnitude of this MutableBigInteger. The magnitude starts at an offset and
	 * offset + intLen may be less than value.length.
	 */
	int intLen;
	 ...
	 * BigDecimal divideAndRound to increment the quotient. Use this constant only
	 * when the method is not going to modify this object.
	 */
	static final MutableBigInteger ONE = new MutableBigInteger(1);

	/**
	 * The minimum {@code intLen} for cancelling powers of two before dividing. If
	 * the number of ints is less than this threshold, {@code divideKnuth} does
	 * not eliminate common powers of two from the dividend and divisor.
	 */
	static final int KNUTH_POW2_THRESH_LEN = 6;

	/**
	 * The minimum number of trailing zero ints for cancelling powers of two
	 * before dividing. If the dividend and divisor don't share at least this many
	 * zero ints at the end, {@code divideKnuth} does not eliminate common powers
	...
	/**
	 * Construct a new MutableBigInteger with the specified value array up to the
	 * length of the array supplied.
	 */
	MutableBigInteger(int[] val) {
		value = val;
		intLen = val.length;
	...
	/**
	 * Makes this number an {@code n}-int number all of whose bits are ones. Used
	 * by Burnikel-Ziegler division.
	 * 
	 * @param n
	...
	/**
	 * Convert this MutableBigInteger to a long value. The caller has to make sure
	 * this MutableBigInteger can be fit into long.
	 */
	private long toLong() {
	...
	/**
	 * This is for internal use in converting from a MutableBigInteger object into
	 * a long value given a specified sign. returns INFLATED if value is not fit
	 * into long
	 */
	 ...
	 * MutableBigInteger is numerically less than, equal to, or greater than
	 * <tt>b</tt>.
	 */
	 ...
	 * {@code b.leftShift(32*ints); return compare(b);} would return, but doesn't
	 * change the value of {@code b}.
	 */
	...
	/**
	 * Convert this MutableBigInteger into an int array with no leading zeros, of
	 * a length that is equal to this MutableBigInteger's intLen.
	 */
	int[] toIntArray() {
	...
	/**
	 * Right shift this MutableBigInteger n bits. The MutableBigInteger is left in
	 * normal form.
	 */
	...
	void leftShift(int n) {
		 ...
		 * value array is faster to utilize, so the extra space will be taken from
	 ...
	 * word input x, and subtracts the n word product from q. This is needed when
	...
	/**
	 * Right shift this MutableBigInteger n bits, where n is less than 32. Assumes
	 * that intLen > 0, n > 0 for speed
	 */
	...
	/**
	 * Left shift this MutableBigInteger n bits, where n is less than 32. Assumes
	 * that intLen > 0, n > 0 for speed
	 */
	...
	private BigInteger getLower(int n) {
		if (isZero()) {
			...
			return new BigInteger(
					Arrays.copyOfRange(value, offset + intLen - len, offset + intLen),
					sign);
		}
	}
	 ...
	 * within this MutableBigInteger. The contents of the addend are not changed.
	 */
	...
	void addShifted(MutableBigInteger addend, int n) {
		...
		while (x > 0 && y > 0) {
			x--;
			y--;
			int bval = y + addend.offset < addend.value.length
					? addend.value[y + addend.offset] : 0;
	...
	/**
	 * Subtracts the smaller of a and b from the larger and places the result into
	 * the larger. Returns 1 if the answer is in a, -1 if in b, 0 if no operation
	 * was performed.
	 */
	...
	void multiply(MutableBigInteger y, MutableBigInteger z) {
		...
		for (int i = xLen - 2; i >= 0; i--) {
			...
			for (int j = yLen - 1, k = yLen + i; j >= 0; j--, k--) {
				long product = (y.value[j + y.offset] & LONG_MASK)
						* (value[i + offset] & LONG_MASK) + (z.value[k] & LONG_MASK)
						+ carry;
				z.value[k] = (int) product;
				carry = product >>> 32;
			}
	...
	MutableBigInteger divideAndRemainderBurnikelZiegler(MutableBigInteger b,
		...
		if (r < s) {
			...
			// Unlike Knuth division, we don't check for common powers of two here
			// because
			...
			int m = 1 << (32 - Integer
					.numberOfLeadingZeros(s / BigInteger.BURNIKEL_ZIEGLER_THRESHOLD));
			...
			bShifted.safeLeftShift(sigma); // step 4a: shift b so its length is a
																			// multiple of n
			MutableBigInteger aShifted = new MutableBigInteger(this);
			aShifted.safeLeftShift(sigma); // step 4b: shift a by the same amount

			// step 5: t is the number of blocks needed to accommodate a plus one
			// additional bit
			...
			MutableBigInteger z = aShifted.getBlock(t - 2, t, n); // the second to
																														// most significant
																														// block
			z.addDisjoint(a1, n); // z[t-2]

			// do schoolbook division on blocks, dividing 2-block numbers by 1-block
			// numbers
			...
			// final iteration of step 8: do the loop one more time for i=0 but leave
			// z unchanged
			ri = z.divide2n1n(bShifted, qi);
			quotient.add(qi);
	...
	private MutableBigInteger divide3n2n(MutableBigInteger b,
		...
		// However, don't subtract d until after the while loop so r doesn't become
		// negative
		r.leftShift(32 * n);
		r.addLower(this, n);
	...
	private MutableBigInteger divideMagnitude(MutableBigInteger div,
		...
		if (shift > 0) {
			...
			divisor = Arrays.copyOfRange(div.value, div.offset,
					div.offset + div.intLen);
		...
		if (qhat != 0) {
			...
			if (needRemainder)
				...
				borrow = mulsubBorrow(rem.value, divisor, qhat, dlen,
						limit - 1 + rem.offset);
	 ...
	 * value. The quotient will be placed into the provided quotient object & the
	...
	/**
	 * This method is used for division by long. Specialized version of the method
	 * sulsub. dh is a high part of the divisor, dl is a low part
	 */
	...
	/**
	 * Compare two longs as if they were unsigned. Returns true iff one is bigger
	 * than two.
	 */
	 ...
	 * multi precision numbers. It is used when the signed value of n is less than
	 * zero. Returns long value where high 32 bits contain remainder value and low
	 * 32 bits contain quotient value.
	 */
	 ...
	 * "Montgomery Form"). The algorithm is described in an unpublished manuscript
	 * entitled "Fast Modular Reciprocals."
	 */
	private MutableBigInteger modInverse(MutableBigInteger mod) {
		...
		while (!f.isOne()) {
			...
			// If f == g (mod 4)
			if (((f.value[f.offset + f.intLen - 1] ^ g.value[g.offset + g.intLen - 1])
					& 3) == 0) {
				f.subtract(g);
				c.signedSubtract(d);
			} else { // If f != g (mod 4)
	...
	/**
	 * Uses the extended Euclidean algorithm to compute the modInverse of base mod
	 * a modulus that is a power of 2. The modulus is 2^k.
	 */
