...
public class BigInteger extends Number implements Comparable<BigInteger> {
	 ...
	 * {@code mag[0]}) must be non-zero. This is necessary to ensure that there is
	 ...
	 * @serial
	 * @see #bitCount
	 * @deprecated Deprecated since logical value is offset from stored value and
	 *             correction factor is applied in accessor method.
	 */
	@Deprecated
	private int bitCount;

	/**
	 * One plus the bitLength of this BigInteger. Zeros means unitialized. (either
	 * value is acceptable).
	 *
	 * @serial
	 * @see #bitLength()
	...
	/**
	 * The threshold value for using 3-way Toom-Cook multiplication. If the number
	 * of ints in each mag array is greater than the Karatsuba threshold, and the
	 * number of ints in at least one of the mag arrays is greater than this
	 ...
	 * the number are larger than this value, Karatsuba squaring will be used.
	 * This value is found experimentally to work well.
	 */
	private static final int KARATSUBA_SQUARE_THRESHOLD = 128;

	/**
	 * The threshold value for using Toom-Cook squaring. If the number of ints in
	 * the number are larger than this value, Toom-Cook squaring will be used.
	 * This value is found experimentally to work well.
	 */
	...
	/**
	 * The offset value for using Burnikel-Ziegler division. If the number of ints
	 * in the divisor exceeds the Burnikel-Ziegler threshold, and the number of
	 * ints in the dividend is greater than the number of ints in the divisor plus
	...
	/**
	 * The threshold value for using Schoenhage recursive base conversion. If the
	 * number of ints in the number are larger than this value, the Schoenhage
	 * algorithm will be used. In practice, it appears that the Schoenhage routine
	 * is faster for any threshold down to 2, and is relatively flat for
	 * thresholds between 2-25, so this choice may be varied within this range for
	 * very small effect.
	 */
	private static final int SCHOENHAGE_BASE_CONVERSION_THRESHOLD = 20;

	/**
	 * The threshold value for using squaring code to perform multiplication of a
	 * {@code BigInteger} instance by itself. If the number of ints in the number
	 * are larger than this value, {@code multiply(this)} will return
	 * {@code square()}.
	 */
	 ...
	 * assumed to be in <i>big-endian</i> byte-order: the most significant byte is
	 ...
	 * element. A zero-length magnitude array is permissible, and will result in a
	 ...
	 * @throws NumberFormatException
	 *           {@code signum} is not one of the three legal values (-1, 0, and
	 *           1), or {@code signum} is 0 and {@code magnitude} contains one or
	 *           more non-zero bytes.
	 */
	 ...
	 * radix. The character-to-digit mapping is provided by {@code
	...
	private static long bitsPerDigit[] = { 0, 0, 1024, 1624, 2048, 2378, 2648,
			2875, 3072, 3247, 3402, 3543, 3672, 3790, 3899, 4001, 4096, 4186, 4271,
			4350, 4426, 4498, 4567, 4633, 4696, 4756, 4814, 4870, 4923, 4975, 5025,
	...
	/**
	 * Constructs a randomly generated BigInteger, uniformly distributed over the
	 * range 0 to (2<sup>{@code numBits}</sup> - 1), inclusive. The uniformity of
	 * the distribution assumes that a fair source of random bits is provided in
	 * {@code rnd}. Note that this constructor always constructs a non-negative
	 * BigInteger.
	 *
	 * @param numBits
	 ...
	 * used in preference to this constructor unless there is a compelling need to
	 * specify a certainty.
	 *
	 * @param bitLength
	 ...
	 * @param rnd
	 *          source of random bits used to select candidates to be tested for
	 *          primality.
	 ...
	 * This method is used for smaller primes, its performance degrades on larger
	 * bitlengths.
	 *
	...
	/**
	 * Find a random number of the specified bitLength that is probably prime.
	 * This method is more appropriate for larger bitlengths since it uses a sieve
	 * to eliminate most composites before using a more expensive test.
	 */
	 ...
	 * probably prime. The probability that the number returned by this method is
	 * composite does not exceed 2<sup>-100</sup>. This method will never skip
	 * over a prime when searching: if it returns {@code p}, there is no prime
	 * {@code q} such that {@code this < q < p}.
	 *
		...
		if (result.bitLength() < SMALL_PRIME_THRESHOLD) {
			...
			while (true) {
				...
				if (result.bitLength() > 6) {
					...
					if ((r % 3 == 0) || (r % 5 == 0) || (r % 7 == 0) || (r % 11 == 0)
							|| (r % 13 == 0) || (r % 17 == 0) || (r % 19 == 0)
							|| (r % 23 == 0) || (r % 29 == 0) || (r % 31 == 0)
							|| (r % 37 == 0) || (r % 41 == 0)) {
	...
	/**
	 * Returns {@code true} if this BigInteger is probably prime, {@code false} if
	 ...
	 * @param certainty
	 *          a measure of the uncertainty that the caller is willing to
	 *          tolerate: if the call returns {@code true} the probability that
	 *          this BigInteger is prime exceeds
	 *          {@code (1 - 1/2<sup>certainty</sup>)}. The execution time of this
	 *          method is proportional to the value of this parameter.
	 * @return {@code true} if this BigInteger is probably prime, {@code false} if
	 *         it's definitely composite.
	 */
	 ...
	 * Miller-Rabin tests. This test is taken from the DSA spec (NIST FIPS 186-2).
	 *
	 * The following assumptions are made: This BigInteger is a positive, odd
	...
	/**
	 * This private constructor is for internal use and assumes that its arguments
	 * are correct.
	 */
	...
	/**
	 * Package private methods used by BigDecimal code to add a BigInteger with a
	 * long. Assumes val is not equal to INFLATED.
	 */
	...
	private static BigInteger multiplyByInt(int[] x, int y, int sign) {
		if (Integer.bitCount(y) == 1) {
			return new BigInteger(shiftLeft(x, Integer.numberOfTrailingZeros(y)),
					sign);
		}
		int xlen = x.length;
	...
	BigInteger multiply(long v) {
		...
		if (dh != 0L) {
			...
			for (int i = xlen - 1; i >= 0; i--) {
				long product = (value[i] & LONG_MASK) * dh + (rmag[rstart] & LONG_MASK)
						+ carry;
				rmag[rstart--] = (int) product;
				carry = product >>> 32;
			}
	...
	private static BigInteger multiplyKaratsuba(BigInteger x, BigInteger y) {
		...
		BigInteger result = p1.shiftLeft(32 * half)
				.add(p3.subtract(p1).subtract(p2)).shiftLeft(32 * half).add(p2);
	 ...
	 * is generally only used when numbers become significantly larger.
	 *
	 ...
	 * @param slice
	 *          The index of which slice is requested, which must be a number from
	 *          0 to size-1. Slice 0 is the highest-order bits, and slice size-1
	 *          are the lowest-order bits. Slice 0 may be of different size than
	 *          the other slices.
	 * @param fullsize
	 *          The size of the larger integer array, used to align slices to the
	 *          appropriate position when multiplying different-sized numbers.
	 */
	...
	private static final int[] squareToLen(int[] x, int len, int[] z) {
		 ...
		 * Technique: Consider the partial products in the multiplication of "abcde"
		 * by itself:
		 *
		 * a b c d e * a b c d e ================== ae be ce de ee ad bd cd dd de ac
		 * bc cc cd ce ab bb bc bd be aa ab ac ad ae
		 *
		 * Note that everything above the main diagonal: ae be ce de = (abcd) * e ad
		 * bd cd = (abc) * d ac bc = (ab) * c ab = (a) * b
		 *
		 * is a copy of everything below the main diagonal: de cd ce bc bd be ab ac
		 * ad ae
		 *
		 ...
		 * off-diagonal added, and multiplied by two again. The low bit is simply a
	...
	private BigInteger squareKaratsuba() {
		...
		return xhs.shiftLeft(half * 32)
				.add(xl.add(xh).square().subtract(xhs.add(xls))).shiftLeft(half * 32)
				.add(xls);
	}
	...
	private BigInteger divideKnuth(BigInteger val) {
		MutableBigInteger q = new MutableBigInteger(),
				a = new MutableBigInteger(this.mag), b = new MutableBigInteger(val.mag);
	 ...
	 * @throws ArithmeticException
	 *           {@code exponent} is negative. (This would cause the operation to
	...
	/**
	 * Left shift int array a up to len by n bits. Returns the array that results
	 * from the shift since space may have to be reallocated.
	 */
	 ...
	 * <tt>(this<sup>exponent</sup> mod m)</tt>. (Unlike {@code pow}, this method
	 ...
	 * @throws ArithmeticException
	 *           {@code m} &le; 0 or the exponent is negative and this BigInteger
	...
	public BigInteger modPow(BigInteger exponent, BigInteger m) {
		...
		BigInteger base = (this.signum < 0 || this.compareTo(m) >= 0 ? this.mod(m)
				: this);
		BigInteger result;
		if (m.testBit(0)) { // odd modulus
			...
			if (m.mag.length < MAX_MAG_LENGTH / 2) {
				result = a1.multiply(m2).multiply(y1).add(a2.multiply(m1).multiply(y2))
						.mod(m);
			} else {
				MutableBigInteger t1 = new MutableBigInteger();
				new MutableBigInteger(a1.multiply(m2))
						.multiply(new MutableBigInteger(y1), t1);
				MutableBigInteger t2 = new MutableBigInteger();
				new MutableBigInteger(a2.multiply(m1))
						.multiply(new MutableBigInteger(y2), t2);
				t1.add(t2);
				MutableBigInteger q = new MutableBigInteger();
	...
	private BigInteger oddModPow(BigInteger y, BigInteger z) {
		 ...
		 * n^(high-order bits of exp) and then keep appending exponent bits to it.
		 * The following patterns apply to a 3-bit window (k = 3): To append 0:
		 * square To append 1: square, multiply by n^1 To append 10: square,
		 * multiply by n^1, square To append 11: square, square, multiply by n^3 To
		 * append 100: square, multiply by n^1, square, square To append 101:
		 * square, square, square, multiply by n^5 To append 110: square, square,
		 * multiply by n^3, square To append 111: square, square, square, multiply
		 * by n^7
		 *
		 * Since each pattern involves only one multiply, the longer the pattern the
		 * better, except that a 0 (no multiplies) can be appended directly. We
		 * precompute a table of odd powers of n, up to 2^k, and can then multiply k
		 * bits of exponent at a time. Actually, assuming random exponents, there is
		 * on average one zero bit between needs to multiply (1/2 of the time
		 * there's none, 1/4 of the time there's 1, 1/8 of the time, there's 2, 1/32
		 * of the time, there's 3, etc.), so you have to do one multiply per k+1
		 * bits of exponent.
		 *
		 ...
		 * rather than placing n into the buffer and then starting to square it, we
		 * have already computed n^2 to compute the odd-powers table, so we can
		 * place that into the buffer and save a squaring.
		 *
		 * This means that if you have a k-bit window, to compute n^z, where z is
		 * the high k bits of the exponent, 1/2 of the time it requires no
		 * squarings. 1/4 of the time, it requires 1 squaring, ... 1/2^(k-1) of the
		 * time, it reqires k-2 squarings. And the remaining 1/2^(k-1) of the time,
		 * the top k bits are a 1 followed by k-1 0 bits, so it again only requires
		 * k-2 squarings, not k-1. The average of these is 1. Add that to the one
		 * squaring we have to do to compute the table, and you'll see that a k-bit
		 * window saves k-2 squarings as well as reducing the multiplies. (It
		 * actually doesn't hurt in the case k = 1, either.)
		 */
	...
	static int mulAdd(int[] out, int[] in, int offset, int len, int k) {
		...
		for (int j = len - 1; j >= 0; j--) {
			long product = (in[j] & LONG_MASK) * kLong + (out[offset] & LONG_MASK)
					+ carry;
	...
	private BigInteger modPow2(BigInteger exponent, int p) {
		/*
		 * Perform exponentiation using repeated squaring trick, chopping off high
		 * order bits as indicated by modulus.
		 */
	 ...
	 * BigInteger (the number of zero bits to the right of the rightmost one bit).
	 * Returns -1 if this BigInteger contains no one bits. (Computes
	...
	/**
	 * Returns {@code true} if this BigInteger is probably prime, {@code false} if
	 * it's definitely composite. If {@code certainty} is &le; 0, {@code true} is
	 * returned.
	 *
	 * @param certainty
	 *          a measure of the uncertainty that the caller is willing to
	 *          tolerate: if the call returns {@code true} the probability that
	 *          this BigInteger is prime exceeds (1 - 1/2<sup>
	 ...
	 * @return 2015年12月8日
	 * @author Jiupeng
	 * @description To optimize the compare method, I choose not to compare all of
	 *              elements in the array. If the length of array is large enough,
	 *              I could omit the lower order
	 */
	final int compareMagnitude(BigInteger val) {
		...
		if (len1 > len2)
			return 1;
		int len_round = Math.round(len1 * threhold);
		for (int i = 0; i < len_round; i++) {
			int a = m1[i];
			int b = m2[i];
			if (a != b)
	...
	private String smallToString(int radix) {
		...
		while (tmp.signum != 0) {
			...
			MutableBigInteger q = new MutableBigInteger(),
					a = new MutableBigInteger(tmp.mag), b = new MutableBigInteger(d.mag);
	...
	private static void toString(BigInteger u, StringBuilder sb, int radix,
		...
		n = (int) Math
				.round(Math.log(b * LOG_TWO / logCache[radix]) / LOG_TWO - 1.0);
	...
	/**
	 * Returns the value radix^(2^exponent) from the cache. If this value doesn't
	 * already exist in the cache, it is added.
	 * <p/>
	 ...
	 * BigInteger. The byte array will be in <i>big-endian</i> byte-order: the
	...
	public double doubleValue() {
		...
		long bits = (long) ((exponent
				+ DoubleConsts.EXP_BIAS)) << (DoubleConsts.SIGNIFICAND_WIDTH - 1);
		bits += signifRounded;
		/*
	 ...
	 */
	private static int digitsPerInt[] = { 0, 0, 30, 19, 15, 13, 11, 11, 10, 9, 9,
			8, 8, 8, 8, 7, 7, 7, 7, 7, 7, 7, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6,
			5 };

	private static int intRadix[] = { 0, 0, 0x40000000, 0x4546b3db, 0x40000000,
			0x48c27395, 0x159fd800, 0x75db9c97, 0x40000000, 0x17179149, 0x3b9aca00,
			0xcc6db61, 0x19a10000, 0x309f1021, 0x57f6c100, 0xa2f1b6f, 0x10000000,
			0x18754571, 0x247dbc80, 0x3547667b, 0x4c4b4000, 0x6b5a6e1d, 0x6c20a40,
			0x8d2d931, 0xb640000, 0xe8d4a51, 0x1269ae40, 0x17179149, 0x1cb91000,
			0x23744899, 0x2b73a840, 0x34e63b41, 0x40000000, 0x4cfa3cc1, 0x5c13d840,
	 ...
	 * array is discarded. Note: The current convention is to initialize the cache
	 * fields, bitCount, bitLength and lowestSetBit, to 0 rather than some other
	 * marker value. Therefore, no explicit action to set these fields needs to be
	 * taken in readObject because those fields already have a 0 value be default
	 * since defaultReadObject is not being used.
	 */
	private void readObject(java.io.ObjectInputStream s)
			throws java.io.IOException, ClassNotFoundException {
		/*
		 * In order to maintain compatibility with previous serialized forms, the
		 * magnitude of a BigInteger is serialized as an array of bytes. The
		 * magnitude field is used as a temporary store for the byte array that is
		 * deserialized. The cached computation fields should be transient but are
		 * serialized for compatibility reasons.
		 */
	...
	private static class UnsafeHolder {
		...
		static {
			try {
				...
				magOffset = unsafe
						.objectFieldOffset(BigInteger.class.getDeclaredField("mag"));
			} catch (Exception ex) {
	 ...
	 *
	 * @serialData two necessary fields are written as well as obsolete fields for
	 ...
	 * information. If the value of this {@code BigInteger} is out of the range of
	 * the {@code long} type, then an {@code ArithmeticException} is thrown.
	 *
	 ...
	 * information. If the value of this {@code BigInteger} is out of the range of
	 * the {@code int} type, then an {@code ArithmeticException} is thrown.
	 *
	 ...
	 * information. If the value of this {@code BigInteger} is out of the range of
	 * the {@code short} type, then an {@code ArithmeticException} is thrown.
	 *
	 ...
	 * information. If the value of this {@code BigInteger} is out of the range of
	 * the {@code byte} type, then an {@code ArithmeticException} is thrown.
	 *
